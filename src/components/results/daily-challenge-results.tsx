...
export function DailyChallengeResults({ totalScore, maxScore = 100 }: DailyChallengeResultsProps) {
  ...
  const handleShare = () => {
    const text = `🏆 Ranked11 Daily Challenge\nMy score: ${formatScore(totalScore)}/100 — Rank #${dailyRank.toLocaleString()}\nPlay at ranked11.app`;
    if (navigator.share) {
    ...
    <div className="min-h-[calc(100dvh-4rem)] pb-12">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center">
          ...
          <h1 className="gold-text font-display font-black tracking-wide">{t("dailySubtitle")}</h1>
          <p className="mt-2 text-xs italic" style={{ color: isDark ? "#64748B" : "#94A3B8" }}>
            {t("previewNotice")}
          </p>
        </motion.div>
        ...
        >
          ...
          <div className="p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              >
                <Flame size={12} />
                <span className="font-display font-bold text-sm">14 {t("dayStreak")}</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-6 flex-wrap">
              ...
              <div className="flex-1 grid grid-cols-2 gap-3 min-w-[200px]">
                {[
                  { icon: Star, val: `${formatScore(totalScore)}/${maxScore}`, label: t("yourScore"), color: "#D4AF37" },
                  { icon: TrendingUp, val: formatScore(totalScore), label: "PTS", color: "#22C55E" },
                  { icon: Users, val: "24.8K", label: t("playedTodayLabel"), color: "#3B82F6" },
                  {
                    icon: Trophy,
          ...
          {LEADERBOARD.slice(0, 5).map((p, i) => (
            ...
            >
              <div className="w-7 text-center font-display font-black text-base">
                {RANK_EMOJIS[p.rank] ?? (
                  <span style={{ color: isDark ? "#64748B" : "#94A3B8" }}>{p.rank}</span>
                )}
              </div>
              <div
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[0.7rem]" style={{ color: isDark ? "#64748B" : "#94A3B8" }}>
                  {p.flag} {p.country}
                </div>
              </div>
          ...
          <div className="flex gap-3 justify-center">
            {[
              ...
              <div
                ...
                style={{
                  background: isDark ? "rgba(30,41,59,0.8)" : "rgba(15,23,42,0.06)",
                }}
              >
