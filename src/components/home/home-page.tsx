          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              ...
              >
                ...
                <span className="gold-text font-display font-bold text-xs tracking-widest">{t("heroEyebrow")}</span>
              </div>
              <h1 className="gold-text mb-4 font-display font-black text-5xl tracking-tight">{t("heroTitle")}</h1>
              <p className="mb-8 max-w-md leading-relaxed" style={{ color: muted }}>{t("heroSub")}</p>
                ...
                <div className="relative z-[1] p-6">
                  <div className="flex items-center justify-between mb-5">
                    ...
                    >
                      ...
                      <span className="font-display font-bold text-[0.7rem] tracking-wider">{t("live")}</span>
                    </div>
                  </div>
                  <div className="rounded-xl p-4 mb-5 text-center glass-daily-inset">
                    <div className="text-4xl mb-2">🏆</div>
                    <div className="font-display font-black text-3xl tracking-widest" style={{ color: "#D4AF37" }}>
                      ???
                    </div>
                    <p className="text-xs mt-1" style={{ color: muted }}>{t("dailyDesc")}</p>
                  </div>
                  <div className="mb-5">
                    <div className="flex items-center gap-1.5 mb-2 text-xs" style={{ color: muted }}>
                      <Clock size={12} />
                      <span className="font-display font-semibold tracking-wider">{t("countdown")}</span>
                    </div>
                    <div className="flex gap-2">
                      {[
                        ...
                        <div key={label} className="flex-1 rounded-xl p-3 text-center glass-daily-inset">
                          <div className="gold-text font-display font-extrabold text-2xl">{val}</div>
                          <div className="text-[0.6rem] font-display tracking-widest" style={{ color: muted }}>
                            {label}
                          </div>
                        </div>
                      ))}
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 py-12">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-display font-extrabold tracking-wider" style={{ color: fg }}>
              {t("topPlayers")}
            </h2>
          </div>
          <Link href="/leaderboard" className="flex items-center gap-1 text-sm font-display font-semibold tracking-wider text-primary">
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          {LEADERBOARD.slice(0, 3).map((p, i) => (
            <motion.div
              ...
              style={{ background: isDark ? "rgba(15,23,42,0.8)" : "rgba(255,255,255,0.9)" }}
            >
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <div className="font-display font-bold text-sm" style={{ color: fg }}>{p.name}</div>
              ...
              <div className="flex justify-between">
                <div>
                  <div className="gold-text font-display font-extrabold text-xl">{p.score.toLocaleString()}</div>
                  <div className="text-[0.7rem] font-display tracking-wider" style={{ color: muted }}>
                    {t("score").toUpperCase()}
                  </div>
                </div>
                <div className="text-right">
                  <div className="flex items-center gap-1 justify-end font-display font-bold text-lg" style={{ color: "#22C55E" }}>
                    <Flame size={13} />
                    {p.streak}
                  </div>
                  <div className="text-[0.7rem] font-display tracking-wider" style={{ color: muted }}>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 py-8 pb-20">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="ml-4 text-sm" style={{ color: muted }}>{t("categoriesSub")}</p>
          </div>
          <Link href="/categories" className="hidden sm:flex items-center gap-1 text-sm font-display font-semibold tracking-wider text-primary">
            {t("viewAll")} <ChevronRight size={14} />
          </Link>
        </div>
          ...
          {featured.map((cat, i) => (
            <Link key={cat.id} href={`/categories/${cat.id}`}>
              <motion.div
                ...
                style={{ background: isDark ? "rgba(15,23,42,0.8)" : "rgba(255,255,255,0.9)" }}
              >
                <div className="mb-1 font-display font-bold" style={{ color: fg }}>{cat.title}</div>
                <p className="mb-4 text-xs leading-relaxed line-clamp-2" style={{ color: muted }}>
                  {cat.description}
                </p>
                <span
                  className="px-2.5 py-0.5 rounded-full text-xs font-bold font-display tracking-wider"
                  style={{
                    background: `${DIFF_COLORS[cat.difficulty] ?? DIFF_COLORS.Medium}20`,
                    color: DIFF_COLORS[cat.difficulty] ?? DIFF_COLORS.Medium,
                  }}
                >
                  {cat.difficulty}
                </span>
